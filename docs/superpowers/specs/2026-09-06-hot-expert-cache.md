...
首轮只证明运行机制和当前负载收益，不预设 50% 最优。Mac 只执行 CPU 测试；CUDA 正确性和端到端性能必须由服务器验证。服务器无正常互联网：沿用已固定的 Docker Hub 镜像和仓库 wheel，不增加在线安装步骤。模型与公共数据只读，所有输出位于服务器项目 `/home/jovyan/wangtonghan/moe-flex` 内。
